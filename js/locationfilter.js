/*
 * Leaflet.locationfilter for Leaflet 1.7
 * cribbed & modified from the original by Tripbirds; see original copyright and attribution below
 */
/*
...
L.LatLngBounds.prototype.modify = function (map, amount) {
  ...
  ne = map.layerPointToLatLng(
    new L.Point(nePoint.x + amount, nePoint.y - amount)
  );

  return new L.LatLngBounds(sw, ne);
};

L.LocationFilter = L.Control.extend({
  options: {},
  ...
  onAdd: function (map) {
    ...
    // we have no visible container, but we need one to return
  ...
  _drawMoveMarker: function (point) {
    ...
    this._moveMarker.on("drag", function (e) {
      ...
      that._nw = new L.LatLng(
        that._nw.lat + latDelta,
        that._nw.lng + lngDelta,
        true
      );
      that._ne = new L.LatLng(
        that._ne.lat + latDelta,
        that._ne.lng + lngDelta,
        true
      );
      that._sw = new L.LatLng(
        that._sw.lat + latDelta,
        that._sw.lng + lngDelta,
        true
      );
      that._se = new L.LatLng(
        that._se.lat + latDelta,
        that._se.lng + lngDelta,
        true
      );
      that._draw();
    });
  ...
  _setupResizeMarkerTracking: function (marker, options) {
    ...
    marker.on("drag", function (e) {
      ...
      latMarker.setLatLng(
        new L.LatLng(curPosition.lat, latMarker.getLatLng().lng, true)
      );
      lngMarker.setLatLng(
  ...
  _calculateBounds: function () {
    var mapBounds = this._map.getBounds(),
      outerBounds = new L.LatLngBounds(
        ...
        new L.LatLng(
          mapBounds.getNorthEast().lat + 0.1,
    ...
    this._northBounds = new L.LatLngBounds(
      new L.LatLng(this._ne.lat, this._osw.lng, true),
      this._one
    );
    this._westBounds = new L.LatLngBounds(
      new L.LatLng(this._sw.lat, this._osw.lng, true),
      this._nw
    );
    this._eastBounds = new L.LatLngBounds(
      this._se,
      new L.LatLng(this._ne.lat, this._one.lng, true)
    );
    this._southBounds = new L.LatLngBounds(
      this._osw,
      new L.LatLng(this._sw.lat, this._one.lng, true)
    );
  },
  ...
  _initialDraw: function () {
    ...
    this._setupResizeMarkerTracking(this._nwMarker, {
      moveAlong: {lat: this._neMarker, lng: this._swMarker}
    });
    this._setupResizeMarkerTracking(this._neMarker, {
      moveAlong: {lat: this._nwMarker, lng: this._seMarker}
    });
    this._setupResizeMarkerTracking(this._swMarker, {
      moveAlong: {lat: this._seMarker, lng: this._nwMarker}
    });
    this._setupResizeMarkerTracking(this._seMarker, {
      moveAlong: {lat: this._swMarker, lng: this._neMarker}
    });
